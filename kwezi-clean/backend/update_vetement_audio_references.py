#!/usr/bin/env python3
"""
Script pour mettre à jour les références audio pour la section "vetements"
...
def normalize_for_matching(text):
    ...
    return text.lower().replace(' ', '').replace('_', '').replace('-', '').replace("'", '').replace('é', 'e').replace('è', 'e').replace('ê', 'e')

def find_vetement_audio_matches(db, audio_dir):
    """Trouve les correspondances entre les mots de vêtements et les fichiers audio"""
    collection = db['vocabulary']
    
    # Récupérer tous les mots de la section vetements
    words = list(collection.find({"section": "vetements"}))
    logger.info(f"Mots dans la section vêtements: {len(words)}")
    ...
    # Correspondances manuelles spécifiques pour éviter les ambiguïtés
    manual_mappings = {
        "vêtement": "Ngouwo.m4a",
        "salouva": "Salouva.m4a",
        "chemise": "Chimizi.m4a", 
        ...
        "short": "Kaliso.m4a",
        "sous-vêtement": "Silipou.m4a",
        "chapeau": "Kofia.m4a",
        "kamiss": "Kandzou bolé.m4a",
        "boubou": "Ancandzou bé.m4a",
        "haut de salouva": "Body.m4a",  # Probablement pas d'audio dédié
...
def main():
    """Fonction principale"""
    logger.info("Début de la mise à jour des références audio pour la section vêtements")
